...
vim.treesitter.set_query(
    "phpdoc",
    "Alpha_refactor_property_phpdoc_primitives",
    [[
    [
        (type_list
...
vim.treesitter.set_query(
    "phpdoc",
    "Alpha_refactor_property_phpdoc_class",
    [[
    [
        (type_list
            (named_type
            ...
            return {
                {
                    ...
                    action = function()
                        ...
                        if comment_node ~= nil then
                            ...
                            local primitive_query = vim.treesitter.get_query("phpdoc", "Alpha_refactor_property_phpdoc_primitives")
                            ...
                            local class_query = vim.treesitter.get_query("phpdoc", "Alpha_refactor_property_phpdoc_class")
                            for _, match in class_query:iter_matches(test_node, 0) do
                                for _, subNode in pairs(match) do
                                    local type = get_node_text(subNode, 0)
                                    table.insert(property_types, type)
                                end
                        ...
                        -- P(property_types)
                        local getter = generator.getGetter(property_name, nullable, property_types, true)
                        local setter = generator.getSetter(property_name, nullable, property_types, true)
